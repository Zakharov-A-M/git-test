...
class ControllerCatalogProduct extends Controller {
    ...
    protected function validateFormXML() {
        if (!$this->user->hasPermission('modify', 'catalog/product')) {
            $this->error['warning'] = $this->language->get('error_permission');
        }

        if ($this->request->files['file']['type'] != 'text/xml') {
            $this->error['warning'] = $this->language->get('error_type_file');
        }
    ...
    public function importXML()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateFormXML()) {
            header("Content-Type: text/plain");
            $products = file_get_contents($this->request->files['file']['tmp_name']);
            $products = simplexml_load_string($products);
            $this->load->model('catalog/product');

            foreach ($products as $product) {
                $this->model_catalog_product->addProductFromFile($product);
            }
        ...
        $data['export_xml'] = $this->url->link('catalog/product/importXML', 'user_token=' . $this->session->data['user_token'] . $url);

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('catalog/import_product_xml', $data));
    }
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'name'       => $result['name'],
				'model'      => $result['model'],
				'price'      => $this->currency->format($result['price'], $result['currency'] ?? $this->config->get('config_currency')),
				'special'    => $special,
