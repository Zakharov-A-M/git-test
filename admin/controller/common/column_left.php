...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ((string)$this->request->get['user_token'] == $this->session->data['user_token'])) {
            ...
            if ($this->user->hasPermission('access', 'catalog/exchange')) {
                $product[] = array(
                    'name'	   => $this->language->get('text_get_products_1c'),
            ...
            if ($this->user->hasPermission('access', 'catalog/news')) {
                $catalog[] = array(
                    ...
                    'href'     => $this->url->link('catalog/news', 'user_token=' . $this->session->data['user_token']),
                    'children' => array()
                );
            }
