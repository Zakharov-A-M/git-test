...
class ControllerExtensionModuleSubscribe extends Controller
{
	public function index() {
		...
		$result = $this->load->view('extension/module/subscribe', $data);
	...
	public function add() {
		...
		$attempts_info = $this->model_extension_module_subscribe->getAttempts($customer_ip);
		...
		if (!$json) {
			if($this->customer->isLogged()) {	
				...
				if ($customer_info && $customer_info['status'] && ($customer_id == $customer_info['customer_id'])) {
					$this->model_extension_module_subscribe->editSubscribe($customer_id, true);

					$reward_info = $this->model_extension_module_subscribe->getRewards($customer_id, $points_description, $points);
			...
			if(!isset($json['error']) && !$reward_info && $points) {
				$this->model_extension_module_subscribe->addReward($customer_id, $points_description, $points);
			}
			
			$this->model_extension_module_subscribe->addAttempt($customer_mail, $customer_ip);
		}	
